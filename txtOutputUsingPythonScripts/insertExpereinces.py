...
try:
    ...
    with open("experience_insert_statements.txt", "w") as file:
        ...
        # Step 5: Generate experiences and write SQL statements
        experiences_data = []
        ...
        for i in range(num_experiences):
            ...
            file.write(f"INTO Fall24_S003_T8_Experience (Experience_ID, Title, Description, Group_Availability, "
                       f"Min_Group_Size, Max_Group_Size, Pricing, Service_Provider_ID, Schedule_ID) VALUES "
                       f"('{experience_id}', '{title}', '{description}', '{group_availability}', {min_group_size}, "
                       f"{max_group_size}, {pricing}, '{service_provider_id}', '{schedule_id}')\n")

            num_tags = random.randint(1, 2)
            selected_tags = random.sample(tags, num_tags)
            for tag_id, _ in selected_tags:
                experience_tags_data.append((experience_id, tag_id))
                file.write(f"INTO Fall24_S003_T8_Experience_Tags (Experience_ID, Tag_ID) VALUES ('{experience_id}', '{tag_id}')\n")
